...
#                for i in range(6):
                _img_related_para += [
                    self.model.encoder.img_transformer.parameters(),
                    self.model.img_layer.parameters()
                    ]
#                code.interact(local=locals())
#            for n in self.model.encoder.img_transformer.parameters():
#                print("n:", n)
            img_related_para = []
            ...
            for n, p in self.model.named_parameters():
                ...
                for q in img_related_para:
                    if p.shape == q.shape:
                        if torch.equal(p, q): # img
                            flag = 1

                if flag == 0:
                    if any(nd in n for nd in no_decay): # no decay
                        no_decay_p.append(p)
                    else:
#                if not any(nd in n for nd in no_decay) and flag == 0:
                        mt5_para.append(p)
                        continue

            optimizer_grouped_parameters = [
                {
#                    "params": [p for n, p in self.model.named_parameters() if any(nd in n for nd in no_decay) and p not in img_related_para],
                    'params': no_decay_p,
                    "weight_decay": 0.0,
                },
                {
                ...
                {
                    'params': img_related_para,
                    'lr': self.args.learning_rate * self.args.img_lr_factor,
                    "weight_decay": self.args.weight_decay,
                },
            ]
            ''' 
