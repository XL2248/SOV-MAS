...
class T5Attention(nn.Module):
    def __init__(self, config: MMT5Config, has_relative_attention_bias=False):
        super().__init__()
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=4, dim_common=256, n_attn_heads=1, testing=False, evaling=False, training=False):
...
#        cross_attn_type = 0
        print("T5Stack start", self.is_decoder)
        if not self.is_decoder:
            if self.use_img_trans:
                self.img_transformer = ImageTransformerEncoder(embed_tokens=self.embed_tokens, d_model=768, num_layers=6, num_heads=8, dim_feedforward=2048, testing=testing, evaling=evaling, training=training)
    ...
    def forward(
        ...
        seperate_box=None,
        image_mask=None,
        masked_image_features=None,
        masked_image_index=None,
        summary=None,
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            t_v_features = image_features
            img_emb = image_features
            if not self.is_decoder:

                if self.use_img_trans:
#                    print("image_features", image_features.size(), image_len)
                    image_features, t_v_features, img_emb = self.img_transformer(image_features, image_len, seperate_img, seperate_box, summary, summary_attention_mask, masked_image_features, masked_image_index)

                # ==================== text-to-video fusion  =====================
                def forget_gate(image_features, text_features):
                    forget_mask = self.fg(torch.cat((text_features, image_features), 2))
    ...
    #               print("output", output.size(), hidden_states.size())
                    # Residual Connection
...
class T5Model(T5PreTrainedModel):
    ...
    def __init__(self, config: MMT5Config):
        super().__init__(config)
...
@add_start_docstrings("""MMT5 Model with a `language modeling` head on top. """, T5_START_DOCSTRING)
class MMT5ForConditionalGeneration(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight",
        r"lm_head\.weight",
    ...
    def __init__(self, config, fusion_layer=5, use_img_trans=True, use_forget_gate=True, cross_attn_type=0, dim_common=256, n_attn_heads=1, use_image_loss=True, use_com_loss=False, alpha=0.1, beta=0.1, training=False, testing=False, evaling=False):
        super().__init__(config)
        self.model_dim = config.d_model
        print("MT5ForMultiModalGeneration", cross_attn_type)
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        ...
        self.use_com_loss = use_com_loss
        self.alpha = alpha
        self.beta = beta
        self.training = training

        self.encoder = T5Stack(encoder_config, self.shared, fusion_layer, use_img_trans, use_forget_gate, cross_attn_type, dim_common, n_attn_heads, testing, evaling, training)
        if self.use_img_trans:
            self.img_layer = nn.Linear(2048, config.d_model)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = self.encoder(
                ...
                masked_image_index=masked_image_index,
                summary=summary,
            ...
            encoder_outputs = BaseModelOutput(
                ...
                hidden_states=encoder_outputs[1] if len(encoder_outputs) > 1 else None,
                attentions=encoder_outputs[2] if len(encoder_outputs) > 2 else None,
                image_features=encoder_outputs["image_features"],
                image_mask=encoder_outputs["image_mask"],
                t_v_features=encoder_outputs["t_v_features"],
                img_emb=encoder_outputs["img_emb"],
            )
            image_mask=encoder_outputs["image_mask"]
...
#        img_emb = encoder_outputs["img_emb"]
        ...
        if self.model_parallel:
            ...
            if decoder_attention_mask is not None:
                decoder_attention_mask = decoder_attention_mask.to(self.decoder.first_device)
#        print("hidden_states, attention_mask", hidden_states.size(), attention_mask.size())
        # Decode
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
        ...
        lm_logits = self.lm_head(sequence_output)
        #print("image_features, image_mask", image_features.size(), image_mask.size())
        # for image loss
        if self.use_image_loss and self.training:
            #image_features = self.img_layer(image_features)
            img_decoder_outputs = self.decoder(
                input_ids=decoder_input_ids,
                attention_mask=decoder_attention_mask,
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
            ...
            img_sequence_output = img_decoder_outputs[0]

        # Set device for model parallelism
            if self.model_parallel:
                torch.cuda.set_device(self.encoder.first_device)
                self.lm_head = self.lm_head.to(self.encoder.first_device)
                img_sequence_output = img_sequence_output.to(self.lm_head.weight.device)

            if self.config.tie_word_embeddings:
            # Rescale output before projecting on vocab
            # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/transformer/transformer.py#L586
                img_sequence_output = img_sequence_output * (self.model_dim ** -0.5)

            img_lm_logits = self.lm_head(img_sequence_output)
        ...
        if labels is not None:
            ...
            # TODO(thom): Add z_loss https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L666
            if self.use_image_loss and self.training:
                loss += self.alpha * loss_fct(img_lm_logits.view(-1, img_lm_logits.size(-1)), labels.view(-1))
                # region kl loss
                t_v_features = F.log_softmax(t_v_features, dim=-1)
                mrm_loss = F.kl_div(t_v_features.double(),
                                    img_emb.double().squeeze(1),
                                    reduction='batchmean')
                loss += self.beta * mrm_loss
#                code.interact(local=locals())
        ...
        return Seq2SeqLMOutput(
            loss=loss,
            logits=lm_logits, # + self.alpha * img_lm_logits,
            past_key_values=decoder_outputs.past_key_values,
            decoder_hidden_states=decoder_outputs.hidden_states,
            decoder_attentions=decoder_outputs.attentions,
            cross_attentions=decoder_outputs.cross_attentions,
