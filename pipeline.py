...
from transformers import (
    ...
    MMT5ForConditionalGeneration,
    set_seed,
    EncoderDecoderModel,
)
from transformers.trainer_utils import EvaluationStrategy, is_main_process, get_last_checkpoint
...
class ModelArguments:
    ...
    alpha: Optional[float] = field(default=0.1, metadata={"help": "# loss factor"})
    beta: Optional[float] = field(default=0.1, metadata={"help": "# loss factor"})
...
class DataTrainingArguments:
    ...
    max_img_len: Optional[int] = field(
        default=180,
        metadata={
            "help": "the maximun length for video features "
            "than this will be truncated, sequences shorter will be padded."
        },
    )
...
def main():
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not training_args.overwrite_output_dir:
        ...
        if last_checkpoint is None and len(os.listdir(training_args.output_dir)) > 0:
            ...
            #os.removedirs(training_args.output_dir + '/')
#            raise ValueError(
...
#        model = MT5ForConditionalGeneration.from_pretrained(
            ...
            config=config,
            cache_dir=model_args.cache_dir,
            fusion_layer=training_args.fusion_layer,
            use_img_trans=training_args.use_img_trans,
            use_forget_gate=training_args.use_forget_gate,
            cross_attn_type=training_args.cross_attn_type,
            dim_common=training_args.dim_common,
            n_attn_heads=training_args.n_attn_heads,
            alpha=model_args.alpha,
            beta=model_args.beta,
            training=training_args.do_train,
            testing=training_args.do_predict,
            evaling=training_args.do_eval,
        )
    ...
    dataset_collator = Seq2SeqDataCollator(tokenizer, data_args, None, training_args.tpu_num_cores, data_args.max_img_len)
    ...
    if data_args.upsampling_factor is not None:
        ...
        logger.info("upsampling_factor MultiDataset data")
        train_dataset = (
            MultiDataset(
                tokenizer,
                type_path="train",
        ...
        logger.info("monolingual Seq2SeqDataset data ")
        train_dataset = (
            dataset_class(
                tokenizer,
                type_path="train",
...
#    if training_args.cross_attn_type:
#        dataset_class = Seq2SeqMultimodalDataset
    eval_dataset = (
        dataset_class(
            tokenizer,
            type_path="val",
